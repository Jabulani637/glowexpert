...
const { findByEmail, findByCellphone, createUser, updateFailedLogin, resetFailedLogin, findByGoogleId } = require('../models/User');
...
async function login(req, res) {
  ...
  // Only admins use OTP for this demo.
  if (user.role !== 'admin') {
  ...
  const { pool } = require('../db');
  await pool.query(
    `UPDATE users SET otp_code = $1, otp_expires_at = $2, otp_attempts = 0, otp_last_sent_at = NOW() WHERE id = $3`,
    [otp, expiresAt, user.id]
  );

  logAdminEvent(`OTP sent for admin login (email=${user.email}, ip=${ip})`);

  // Return otp in UI for demo.
  return res.status(200).json({
    message: 'OTP required',
    otp_required: true,
    otp,
    token_type: null,
    user: { id: user.id, name: user.name, email: user.email, cellphone: user.cellphone, role: user.role }
  });
}
